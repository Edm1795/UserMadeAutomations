...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        # Default Buttons
        self.createButton = Button(self.frame1, text="Create", width=12, command=lambda:self.createAutomation()) # Button for creating a new automation
        ...
        while runMainLoop:  # loop for gathering input from user. Stopping this loop will move out of the user gathering mode and into run mode
            ...
            if mainRawInput == '1':  # if user wants to add mouse moves
                ...
                if rawInput == '1':  # if adding colour check
                    print(
                        'Use Main Monitor Only: Place mouse over the top of a stable coloured element of the program or website -- 5 seconds\n')
                    time.sleep(5)
